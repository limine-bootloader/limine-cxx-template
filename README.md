...
Assembly files with the `*.S` extension are built using the same toolchain as the C and C++ sources. Only `*.asm` files, which are built for `x86_64` alone and of which the template ships none, require `nasm`. The `run` targets require `qemu`.
